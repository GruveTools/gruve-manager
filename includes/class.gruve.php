...
class Gruve {
    ...
    public function __construct($options = []) {
        ...
        $this->gruve_dir = $this->home_dir . '/.gruve';
        $this->data_dir = $this->gruve_dir . '/data';
        $this->configs_dir = $this->gruve_dir . '/configs';
        $this->log_file = $this->gruve_dir . '/switcher-' . date('Ymd') . '.log';

        if (!file_exists($this->gruve_dir)) {
            if (!@mkdir($this->gruve_dir)) {
                exit('Unable to continue, cannot setup Gruve Client in ' . $this->gruve_dir . "\n");
            }
        }
        if(!file_exists($this->data_dir)) {
            if (!@mkdir($this->data_dir)) {
                exit('Unable to continue, cannot create Gruve Client data directory ' . $this->data_dir . "\n");
            }
        }
        if(!file_exists($this->configs_dir)) {
            if (!@mkdir($this->configs_dir)) {
               exit('Unable to continue, cannot setup Gruve Client in ' . $this->gruve_dir . "\n");
            }
        }
        ...
        if (!file_exists($this->gruve_dir . '/config.json')) {
        ...
        $this->configs = json_decode(file_get_contents($this->gruve_dir . '/config.json'));
    ...
    public function setup($path = '') {
        ...
        if (empty($path)) {
            $path = $this->home_dir . '/gruve-client';
        }

        $config = json_decode(file_get_contents($path . '/config.sample.json'));

        $symbol = readline("Please enter the symbol for the coin you're currently mining: "); // read the special file to get the user input from keyboard
        $symbol = trim($symbol);
        ...
        copy($this->home_dir . '/local.conf', $this->gruve_dir . '/configs/' . $setup_cfg->config);

        $config->configs = $setup_configs;

        $whattomine_paste = readline("Please go to whattomine.com, enter your rig details, and press 'calculate', then copy and paste the very long web address here:\n");
        ...
        file_put_contents($this->gruve_dir . '/config.json', json_encode($config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
